...
def plot_lic_texture(velocity_func, nx, ny, integrator, steps=300, h=0.2, apply_smoothing=False, colormap='bone', gamma=1.2, upscale_factor=2, save_path=None):
    ...
    # Mask land area (assumed low-velocity region) as black
    land_mask = compute_velocity_magnitude(u, v) < 0.5
    land_mask_up = np.kron(land_mask, np.ones((upscale_factor, upscale_factor)))
    lic_gamma[land_mask_up.astype(bool)] = 0.0
...
plot_lic_texture(
    ...
    colormap='bone',
    gamma=1.2,
    save_path="img/isabel_lic_rk4_100.png"
)
